...
```kotlin
java -jar corda-tools-ha-utilities-4.12.jar generate-internal-tunnel-ssl-keystores -p tunnelStorePass -e tunnelPrivateKeyPassword -t tunnelTrustpass
```
...
```kotlin
java -jar corda-tools-ha-utilities-4.12.jar generate-internal-artemis-ssl-keystores -p artemisStorePass -t artemisTrustpass
```
...
```kotlin
java -jar corda-tools-ha-utilities-4.12.jar import-ssl-key --bridge-keystore-password=bridgeKeyStorePassword --bridge-keystore=./nodesCertificates/nodesUnitedSslKeystore.jks --node-keystores=./entityA/certificates/sslkeystore.jks --node-keystore-passwords=entityAStorePass --node-keystores=./entityB/certificates/sslkeystore.jks --node-keystore-passwords=entityBStorePass
```
